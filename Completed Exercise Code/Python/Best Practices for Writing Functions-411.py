...
def count_letter(content, letter):
    ...
    Args:
        ...
        letter (str): letter to count in `content`. 
...
def standardize(column):
    """ Returns z-scores of a column
    
    Args:
        column (pandas Series) : Column we will standardize
        
    Returns:
        z_score of column: (z_score)
    """
...
def find_mean(values):
    ...
    Args:
      value (iterable or float): A list of numbers
      
    Returns:
      tuple(float): The mean 
    """
    mean = sum(values)/ len(values)
    return mean
...
def add_column(values=None, df=None):
